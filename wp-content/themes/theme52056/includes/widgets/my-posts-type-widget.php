...
function widget($args, $instance) {
	...
	?>
		...
		if ( $instance['date'] ) : ?>
			<time datetime="<?php the_time('Y-m-d\TH:i'); ?>"><?php the_date(); ?> <span class="time"><?php the_time() ?></span></time>
		<?php endif;
		...
		if ( $instance['show_title'] ) : ?>
			<h4 class="post-list_h"><a href="<?php the_permalink(); ?>" rel="bookmark" title="<?php the_title_attribute(); ?>"><?php if ( $instance['show_title_date'] ) {?>[<?php echo get_the_date(); ?>]<?php }else{?><?php the_title(); ?><?php }?></a></h4>
		<?php endif; ?>
